...
public sealed class AvaloniaDispatcherAdapter : IDispatcherAdapter
{
    ...
    /// </summary>
    public UiStore? BoundStore => _boundStore;
    ...
    /// </summary>
    /// <param name="store">The store to unbind. If not the currently-bound
    /// store, this is a no-op.</param>
    public void Unbind(UiStore store)
    {
        ArgumentNullException.ThrowIfNull(store);
        if (!ReferenceEquals(_boundStore, store)) return;
        store.Changed -= _onStoreChanged;
        _boundStore = null;
    }
