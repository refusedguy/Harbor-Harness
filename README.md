...
<p align="center">
  <img src="assets/demo/hero-compressed.gif" alt="Harbor — scripted agent run (streaming markdown, tool call, approval gate; mock LLM, no API keys)" width="100%">
</p>

## Demo

All GIFs are generated by the deterministic `harbor --demo` playback (in-process mock LLM — no API keys, no network):

| | |
|---|---|
| ![Streaming markdown — agent loop diagram](assets/demo/markdown-compressed.gif) | ![Approval gate — bash tool ask/allow](assets/demo/approval-compressed.gif) |
| *Streaming markdown rendering* | *Permission approval gate (bash → ask)* |
| ![Renderer switch — plain TUI](assets/demo/plain-compressed.gif) | |
| *Same pipeline through the plain renderer* | |
...
CI regenerates the GIFs on TUI changes (`.github/workflows/demo.yml`).
