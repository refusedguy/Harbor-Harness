...
/// <summary>
///     TEA bridge for the Terminal.Gui renderer. Wraps <see cref="UiStore" /> +
///     <see cref="TuiEffectHost" /> + <see cref="KeyHandler" /> and exposes a
///     Push / Submit / Toast surface so the legacy <c>TerminalGuiScreen</c>
///     can route events through the shared reducer instead of duplicating
///     state.
/// </summary>
public sealed class TerminalGuiTeaBridge : IDisposable
{
    private readonly ILogger _logger;
    private readonly ConcurrentQueue<string> _toastQueue = new();

    public TerminalGuiTeaBridge(IAgent agent, Func<string, Task>? slash, ILogger logger,
    ...
    /// <summary>Key handler that turns Terminal.Gui KeyCode into UiMsg dispatches.</summary>
