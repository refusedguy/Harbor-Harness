...
- **Встроенные инструменты (18 tools)**: Чтение/запись/редактирование файлов (`read`, `write`, `edit`), выполнение команд (`bash`), поиск (`glob`, `grep`, `ripgrep`, `ls`, `tree`), мульти-агентные задачи (`task`), веб-интеграция (`webfetch`), подсистема плагинов MCP (`mcp`, `read_mcp_resource`, `mcp_prompt`), навыки (`skill`) и патчей (`patch`, `notebook`).
