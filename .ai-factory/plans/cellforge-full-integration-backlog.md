...
- [ ] `CF-B-005` Спроецировать ввод: `Input:InputModel (Text/History/HistoryIndex) ↔ PromptBuffer/PromptHistory/ComposerController`. Файлы: `CF-R`, `Rendering/ComposerController.cs`, `Rendering/PromptBuffer.cs`, `Rendering/PromptHistory.cs`, `src/Harbor.Ui.Framework.State/State/InputModel.cs`. Готово когда: Up/Down истории идут через `InputMsg.HistoryUp/Down`, draft сохраняется/восстанавливается (сейчас свой `PromptHistory` в обход стора).
...
- [ ] `CF-D-004` Реализовать `IUiViewport` для CellForge (по образцу `SpectreUiViewport`, `*UiViewport` в Termina/TerminalGui): ширина/скролл/видимые строки как сервис. Файлы: новый `Rendering/CellForgeViewport.cs`, `VCT`, `src/Harbor.Ui.Framework.Projection/Projection/IUiViewport.cs`. Готово когда: `DefaultUiProjector` + `TimelineLayoutCache` используют один вьюпорт.
...
- [ ] `CF-E-002` Зарегистрировать 7 билтинов в CellForge-реестре + wiring: `EnsureSeeded` при старте рендерера, `RenderToRows` в `CSL`/док-регионах `LayoutTree`, `RouteKey` для focused-панели (`j/k` diagnostics/file-tree, `?` help, `F12` logs, `Enter` file-tree→`read`). Источник: `contrib/tui/Harbor.Tui.SpectreTui/SpectreTuiRenderer.cs:122-134` (порядок `Panels.Register(...)` = слоты Alt+1..9). Приоритеты: P0 `todo-list/diff-preview/token-breakdown/diagnostics`, P1 `help/file-tree/logs`. Готово когда: все 7 видны в CellForge, хоткеи работают через `UiMsg.TogglePanel/FocusPanel` (не напрямую).
- [ ] `CF-E-003` Вынести чистые экстракторы в Framework (убрать Spectre-зависимость `Build` от логики): `TodoListPanel.ExtractTodos`, `DiffPreviewPanel.ExtractRecentChanges`, `DiagnosticsPanel.CollectDiagnostics` → `Harbor.Ui.Framework.{Projection|Services}` как чистые `UiState.Lines → модели`. Обновить Spectre-билтины на новые экстракторы (без смены UX), CellForge кормить теми же. Готово когда: один экстрактор — два потребителя, дублирующегося парсинга нет.
...
- [ ] `CF-E-012` `StatusMappers` целиком → ссылка напрямую (P0, S): 9 чистых мапперов (`StatusToBrushKey`, `ToolCallStatusToBrushKey/Pill`, `SessionStatusToText/BrushKey`, `DurationToText`, `TimeAgo`, `TokensToCompact`, `CostToUsd`) уже framework-free. Убрать дубли (`ToolCallBlock.FormatDuration`), добавить `ChatPalette.ToolPill`. Источник: `src/Harbor.Ui.Framework.ViewModels/Converters/StatusMappers.cs`.
...
- [ ] `CF-F-001` Убрать `ShouldRenderPlacement` заглушку (`CF-R:63-71` отрубает ChatHistory/Input): заставить `ChatHistoryView` (Entries+StreamingText+ThinkingText с префиксами `[user]/[assistant]/[tool]/[result]`) и `InputView` (`> +placeholder+Reverse-курсор`) краситься через CellForge painters. Файлы: `CF-R`, `src/Harbor.Terminal.Abstractions/Views/{ChatHistoryView,InputView}.cs`, `VCT`, `Rendering/PromptRenderer.cs`. Готово когда: все 4 builtin views красятся; `return false` ветка удалена.
...
- [ ] `CF-R-009` Тест-покрытие проекции: `ProjectStateIntoWidgets` + будущие проекторы — каждый маппинг `UiState→VM` покрыт (сейчас +4 поля из CF-B без тестов — дописать первыми). Готово когда: `ProjectStateIntoWidgets` покрыта на 100% строк.
...
2. Каждый таск = отдельная ветка + `dotnet build` (0 warnings) + `dotnet test tests/<Project> -c Release --no-build` по затронутым проектам + arch-тесты при смене референсов. Solution-wide `dotnet test` не гонять (MTP-хрупкость, см. AGENTS.md).
