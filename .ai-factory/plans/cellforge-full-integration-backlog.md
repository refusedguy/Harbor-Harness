...
- [x] `CF-E-002` Cell-native провайдеры + wiring: `Panels/CellForgeBuiltinPanels.cs` + реестр в рендерере (`Panels`, `RegisterBuiltinPanels`, `EnsureSeeded` в `InitializeAsync`) + `CellForgeDockPanel/ChatScreenPanelDock` в CSL (left/right/bottom, `RenderToRows`, `RouteKey` при Focused, `PaintBottomStack`-фолбэк). Тесты: `CellForgeBuiltinPanelsTests` (24) + `PanelWiringTests` (12). Проверено: CellForge 903/903.
...
- [x] `CF-E-011` `DiffPreviewHelper.ExtractDiff → R/DiffPreview.cs` (ctx=2, бюджеты 6/80, сентенелы-константы) + `ToolCallInfo{FilePath,DiffPreview,DiffFull}` + `DiffRenderer`-кап; `Measure/CheapEstimate` зеркалят `Paint` (дифф заменяет output, не складывается). Тесты: `DiffPreviewTests.cs` (14). Проверено в волне (903/903).
- [x] `CF-B-006/C-002/C-003` Скролл через стор: фабрики `PageUp/Down/Line/Top/Bottom/ResetToTail/WheelMsg`, `ApplyStoreState` (offset→ScrollY=max-offset, снап ПОСЛЕ layout по свежему max — pre-layout `_virtual` stale; FullRebuild-anchor override; tail из offset==0), `MeasureMsgs` (Viewport/HistoryMeasured/ScrollClamp), `TimelineWheelTarget`. Тесты: `ScrollStoreTests.cs`. Проверено в волне.
...
- [x] `CF-E-013` `CodeBlock.Tokenize + KeywordsFor(6) → R/CodeTokenizer.cs` (`(text,CellStyle)`-спаны, Span-ядро, `TokenizeLine` + `HighlightFenceBodies`) + встройка в `AssistantMarkdownBlock` (fence-тело, маркеры Dim). Тесты: `CodeTokenizerTests.cs` (30). Проверено: CellForge 958/958.
- [x] `CF-E-016` `HarborDesignTokens.axaml` → `JsonThemeLoader.Default` (`"harbor-terminal"`, 14 слотов + 7 ролей + 3 Cost 1-в-1) + `Parse(json, fallback)`-merge; watcher не тронут. Тесты: `ThemeDefaultTests.cs` (6). Проверено в волне.
- [x] `CF-E-017` Каталоги в палитре: `SlashDefs` 10 + `BuiltinDefs` 10 (литеральные 1:1 копии — матрица запрещает референс на Desktop.Shared), `PaletteIconMap` (ASCII/Nerd), `FindSlash/FindBuiltin/Find`, `ShowDefaultCatalog`. Тесты: `PaletteCatalogTests.cs` (14). Проверено в волне.
