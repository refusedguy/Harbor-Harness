...
public class SlashCommandTreeTests
{
    ...
    {
        var output = await DispatchAsync("/tree", new FakeStore(), null);
    ...
    public async Task HandleAsync_TreeChain_RendersIndentedForestAndMarksCurrent()
    {
        ...
        store.Add(Make("child", "forked work", t0.AddMinutes(1), "root"));
        store.Add(Make("grandchild", "deep dive", t0.AddMinutes(2), "child"));
        store.Add(Make("sibling", "second fork", t0.AddMinutes(3), "root"));

        var output = await DispatchAsync("/tree", store, "child");
    ...
    {
        var store = new FakeStore();
        store.Add(Make("orphan", "lost branch", DateTimeOffset.UtcNow, "missing-parent"));

        var output = await DispatchAsync("/tree", store, null);
