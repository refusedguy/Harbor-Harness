...
public class LspServerSessionIntegrationTests
{
    ...
    public async Task Open_PublishesDiagnostics_DefinitionResolves()
    {
        ...
        try
        {
            ...
            await using var session = await LspServerSession.StartAsync(
            ...
            var diagnostics = session.GetDiagnostics(filePath);
            ...
            var location = await session.FindDefinitionAsync(filePath, 1, 0, CancellationToken.None);
...
public class LspManagerTests
{
    ...
    public async Task MissingServerBinary_DegradesGracefully()
    {
        ...
        var location = await manager.FindDefinitionAsync("/x/a.gst", 0, 0, CancellationToken.None);
    ...
    public async Task UnsupportedFile_IsNoOp()
    {
        ...
        var diagnostics = await manager.GetDiagnosticsAsync("/x/a.wasm");
        await Assert.That(diagnostics).IsEmpty();
        var location = await manager.FindDefinitionAsync("/x/a.wasm", 0, 0, CancellationToken.None);
        await Assert.That(location).IsNull();
    }
}
