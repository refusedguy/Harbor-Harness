...
///     (a) точный вход (?1049h ?25l ?2004h + SGR mouse ?1000h ?1002h ?1006h)
///     и выход (?2004l ?25h ?1049l ?1006l ?1002l ?1000l) через Ctrl+C —
///     mouse grab load-bearing для кликов (гейты, сайдбар) со спринта мыши,
///     paste-only контракт §3.1 мёртв; (b) focus-события и DSR-запросы
///     от терминала игнорируются без влияния на сессию.
/// </summary>
[NotInParallel("pty")]
public sealed class TerminalModesScenarioTests : CellForgePtyScenarioBase
{
    [Test]
    [Timeout(60_000)]
    public async Task ModeLifecycle_AltScreenMouseGrab_OrderedLeave()
    {
        await StartAppAsync(100, 30).ConfigureAwait(false);

        // 1. Exact entry sequence lands atomically at startup (mouse grab included).
        ...
        bool hasLeaveSeq = Session.RawText.Contains("\x1b[?2004l\x1b[?25h\x1b[?1049l\x1b[?1006l\x1b[?1002l\x1b[?1000l", StringComparison.Ordinal);
