...
///         <item>
///             <description>
///                 <b>Absolute contract ceilings</b> — always
...
///         <item>
///             <description>
///                 <b>Relative regression check</b> against the
...
///                 (see .github/workflows/renderer-perf-gate.yml).
///             </description>
///         </item>
///     </list>
/// </summary>
...
public class RendererBenchmarkTests
{
    ...
    {
        var contract = RendererPerformanceContract.Defaults
            .FirstOrDefault(c => c.BackendId == backendId);
        await Assert.That(contract).IsNotNull();

        var result = (await RendererBenchmarkSuite.RunAsync(backendId)).Single();
    ...
    public async Task EveryDefaultBackend_IsCoveredByContracts()
    {
        ...
        foreach (var contract in RendererPerformanceContract.Defaults)
        {
