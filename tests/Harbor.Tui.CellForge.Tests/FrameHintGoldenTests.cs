...
public class FrameHintGoldenTests
{
    ...
    public async Task Hinted_Diff_Output_Is_ByteIdentical_To_FullScan()
    {
        ...
        for (int frame = 0; frame < 24; frame++)
        {
            ...
            full.Frame(cols, rows, true, fxScratch, false);
            hinted.Frame(cols, rows, frame == 0 || inputDriven, fxScratch, true);
        }
    ...
    private sealed class Session
    {
        ...
        public static Session Create(int cols, int rows)
        {
            ...
            var screen = new ScreenSession(new AnsiWriter(backend, true), cols, rows);
        ...
        /// <summary>
        ///     One scripted frame: layout, paint, damage policy, flush.
        ///     Mirrors CellForgeReplRunner.RenderFrameAsync + ApplyFrameDamageHints;
        ///     <paramref name="fx" /> selects the hinted policy (reference runs with false).
        /// </summary>
