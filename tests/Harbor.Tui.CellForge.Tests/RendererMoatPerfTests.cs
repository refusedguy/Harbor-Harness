...
public class RendererMoatPerfTests
{
    ...
    public async Task Diff_Time_500RowTimeline_Hinted_Under_2ms()
    {
        ...
        probe.SteadyFrame(cols, false);
        for (int i = 0; i < 300; i++)
        {
            probe.SteadyFrame(cols, false);
            probe.SteadyFrame(cols, true);
        }
    ...
    /// <summary>
    ///     Benchmark report: full-scan diff with the glow pipeline armed
    ...
    /// <summary>
    ///     Backend that counts bytes without copying — keeps allocation
    ///     probes clean while still exercising the full write path.
    /// </summary>
    ...
    private sealed class Probe
    {
        ...
        {
            Screen = new ScreenSession(new AnsiWriter(new CountingBackend(), true), cols, rows);
        ...
        /// <summary>
        ///     Populates the feed until the virtual content is twice the
        ...
        /// <summary>
        ///     Paints one steady-state frame; when <paramref name="hint" />
        ///     is set only the animated rows are damaged, otherwise full scan.
        /// </summary>
