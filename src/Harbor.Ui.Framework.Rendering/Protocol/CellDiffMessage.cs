...
public readonly record struct CellDiffMessage(int X, int Y, Cell OldCell, Cell NewCell)
{
    ...
    public const int EncodedSize = sizeof(int) /* X */ + sizeof(int) /* Y */ + sizeof(int) + sizeof(uint) + sizeof(uint) + sizeof(ushort) + sizeof(byte) /* OldCell */ + sizeof(int) + sizeof(uint) + sizeof(uint) + sizeof(ushort) + sizeof(byte); /* NewCell */
}
