...
///         <item>
///             <description>
///                 row-hash fast-path — silent rows cost O(1) instead of
///                 O(cols);
///             </description>
///         </item>
///         <item>
///             <description>
///                 FrameHint damage rects — point updates (spinner,
///                 caret blink) scan only hinted area while it stays under 25 % of the
///                 screen, otherwise the engine falls back to the full scan;
///             </description>
///         </item>
///         <item>
///             <description>
///                 cursor elision + SGR delta are delegated to the
///                 writer.
///             </description>
///         </item>
///     </list>
...
public sealed class DiffEngine
{
    ...
    public DiffEngine(int cols, int rows)
    {
        Front = new ScreenBuffer(cols, rows);
    }

    public DiffEngine(ScreenBuffer front)
    {
        Front = front;
    }

    /// <summary>The terminal-mirror buffer.</summary>
    public ScreenBuffer Front { get; private set; }
    ...
    {
        var clipped = damage.Intersect(new Rect(0, 0, Front.Cols, Front.Rows));
    ...
    {
        long area = 0;
        var screen = new Rect(0, 0, Front.Cols, Front.Rows);
    ...
    {
        ArgumentOutOfRangeException.ThrowIfNotEqual(next.Cols, Front.Cols);
        ArgumentOutOfRangeException.ThrowIfNotEqual(next.Rows, Front.Rows);

        bool useHints = _hints.Count > 0
                        && HintArea() < (long)Front.Cols * Front.Rows * HintAreaThreshold;
        ...
        {
            ScanRange(0, 0, Front.Cols, Front.Rows, next, writer);
        }
    ...
    private void ScanRange(int x1, int y1, int x2, int y2, ScreenBuffer next, AnsiWriter writer)
    {
        ...
        for (int y = Math.Max(0, y1); y < bottom; y++)
        {
            ...
            if (Front.IsRowHashValid(y) && next.IsRowHashValid(y)
                                        && Front.RowHash[y] == next.RowHash[y])
            {
                continue;
            }
            ...
            {
                ref readonly var n = ref next.At(x, y);
                var f = Front.At(x, y);
                int width = n.Width;
    ...
    /// <summary>
    ///     Applies the armed pipeline (null → identity) to one cell.
    ///     JIT-inlined null check keeps the unarmed hot path at zero added cost.
    /// </summary>
    ...
    /// <summary>
    ///     True when FRONT equals NEXT cell-for-cell over the whole screen
    ...
    ///     comparison is against the transformed look, not the raw paint.
    /// </summary>
    public bool FrontMatches(ScreenBuffer next)
    {
        if (next.Cols != Front.Cols || next.Rows != Front.Rows)
        {
            return false;
        }
