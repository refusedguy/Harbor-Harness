...
public sealed class ScreenSession
{
    ...
    public ScreenSession(AnsiWriter writer, int cols, int rows, Func<(int Cols, int Rows)>? sizeSource = null)
    {
        ...
        Engine = new DiffEngine(cols, rows);
        _sizeSource = sizeSource;
        CurrentCols = cols;
        CurrentRows = rows;
    }

    public ScreenBuffer Back { get; private set; }

    public ScreenBuffer Front => Engine.Front;

    public DiffEngine Engine { get; }

    /// <summary>
    ///     Lock-free buffer handoff used by <see cref="OfferSwap" /> /
    ///     <see cref="AdoptPendingSwap" /> (renderer-moat hot-swap runtime).
    /// </summary>
    public BufferSwapChain SwapChain { get; } = new();
    ...
    /// <summary>
    ///     Per-frame autoresize check (ratatui policy: the render tick is
    ///     the single point of truth about terminal size).
    /// </summary>
    public void CheckAutoSize()
    {
        ...
        (int cols, int rows) = _sizeSource();
        ApplyResize(cols, rows);
    }

    /// <summary>
    ///     Applies new geometry. Deduplicates; records Erase-in-display
    ///     requirement for horizontal shrink.
    /// </summary>
    public void Resize(int cols, int rows) => ApplyResize(cols, rows);
    ...
    /// </summary>
    public void Damage(in Rect rect) => Engine.FrameHint(in rect);
    ...
    /// </summary>
    public void DamageAll() => Engine.ClearHints();
    ...
    public void OfferSwap(ScreenBuffer back, ScreenBuffer front)
    {
        ...
        SwapChain.Publish(new BufferPair(back, front));
    }
    ...
    public bool AdoptPendingSwap()
    {
        ...
        var retiredBack = Back;
        var retiredFront = Engine.Front;

        Back = offer.Back;
        Engine.SwapFront(offer.Front);
        CurrentCols = Back.Cols;
        CurrentRows = Back.Rows;

        Back.InvalidateAll();
        Engine.Front.InvalidateAll();
        Engine.ClearHints();
        ...
        SwapChain.Return(retiredBack);
        SwapChain.Return(retiredFront);
        return true;
    }
    ...
    /// <summary>
    ///     Starts a frame: swap adoption at the frame boundary, palette
    ///     snapshot pin (theme swaps cannot tear a frame mid-paint), sync-on,
    ///     optional erase-in-display first.
    /// </summary>
    ...
    /// <summary>
    ///     Arms the effect pipeline only when it holds active effects —
    ///     the empty pipeline keeps the engine on its exact classic path
    ///     (zero perf regression on non-effect frames).
    /// </summary>
    private void ArmEffects() =>
        Engine.Effects = Effects.Count > 0 ? Effects : null;
}
