...
public enum Utf8DecodeStatus : byte
{
    ...
    /// <summary>
    ///     Pending broken sequence flushed (emit U+FFFD) and the current
    ///     byte was NOT consumed — feed it through the machine again.
    /// </summary>
...
public struct Utf8IncrementalDecoder
{
    ...
    public Utf8DecodeStatus DecodeStep(byte b, out Rune rune)
    {
        ...
        if (_consumed == 0)
        {
            ...
            int expected = LeadSequenceLength(b);
            if (expected == 0)
            {
        ...
        uint minLength = _expected switch { 2 => 0x80u, 3 => 0x800u, _ => 0x10000u };
        Reset();
