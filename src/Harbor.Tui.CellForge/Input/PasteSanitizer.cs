...
/// <param name="Text">
///     Sanitized paste buffer. On the clean fast path this is
///     the ORIGINAL string reference (zero allocation).
/// </param>
/// <param name="Modified">True when anything was stripped or normalized.</param>
/// <param name="EscapeSequences">
///     ANSI escape sequences removed (CSI, OSC,
///     DCS/SOS/PM/APC, lone ESC).
/// </param>
/// <param name="ControlChars">
///     Control bytes removed (C0 except \n/\t, DEL,
///     C1) including \r normalizations.
/// </param>
...
public static class PasteSanitizer
{
    ...
    /// <summary>
    ///     Bytes that force the sanitize pass: ESC + C0 except \n/\t
    ///     (BEL, CR, SO/SI, …), DEL and the whole C1 range (incl. 0x9B — the
    ///     8-bit CSI intro). \n and \t are legitimate text and never trigger.
    /// </summary>
    private static readonly SearchValues<char> Triggers = BuildTriggers();

    /// <summary>
    ///     Sanitizes a paste buffer: escape sequences and control bytes
    ...
    public static PasteSanitizeResult Sanitize(string text)
    {
        ...
        {
            return new PasteSanitizeResult(text, false, 0, 0);
        }

        int length = Scan(source, default, false, out int escapes, out int controls);
        string result = string.Create(
            length, text,
            static (destination, source) => _ = Scan(source, destination, true, out _, out _));
        return new PasteSanitizeResult(result, true, escapes, controls);
    }
    ...
    /// <summary>
    ///     Single-pass state machine: with <c>write=false</c> it counts
    ///     (output length + stats), with <c>write=true</c> it fills the destination.
    ///     Both passes walk identical state so the length is exact.
    /// </summary>
