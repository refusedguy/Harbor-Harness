...
///         <item>
///             <see cref="Tailscale" /> — TCP to a tailnet peer: a MagicDNS name or 100.x CGNAT
///             address. Reachable from anywhere in the tailnet, never from the public internet;
///             PSK handshake is mandatory on these listeners (defence in depth).
///         </item>
///     </list>
///     <para>
...
public abstract record EndpointDescriptor
{
    ...
    /// <remarks>
    ///     <see cref="Psk" /> carries the optional pre-shared key copied
    ///     from hosts.json so tools like <c>harbor status --all</c> can
    ///     authenticate probes without extra plumbing.
    /// </remarks>
