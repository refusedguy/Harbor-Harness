...
/// </para>
/// <list type="bullet">
///   <item><b>Register a new view</b> — append a custom panel to any
///   <see cref="TuiViewPlacement"/> (status bar, chat history, sidebar, overlay, …).
///   The renderer will repaint it on the events selected by
///   <see cref="BaseTuiRenderer.ShouldRenderPlacement"/>.</item>
///   <item><b>Override a builtin view</b> — register a view with the same id as a builtin
...
///   is skipped when an id is already taken (override-before-builtin).</item>
///   <item><b>Register a custom view model</b> — add state holders that views can bind to
///   by id. The <see cref="ViewModelRegistry"/> auto-binds view ↔ view model by matching
///   <see cref="ITuiView.Id"/> to <see cref="ITuiViewModel.Id"/>.</item>
/// </list>
/// <para>
...
public interface ITuiPlugin
{
    ...
    /// </summary>
    /// <param name="views">The view registry — register <see cref="ITuiView"/> instances
    /// here.</param>
    /// <param name="viewModels">The view model registry — register
    /// <see cref="ITuiViewModel"/> instances here.</param>
