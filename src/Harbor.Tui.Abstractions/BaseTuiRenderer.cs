...
/// </para>
/// <list type="bullet">
///   <item>Registers four builtin views (status bar, chat history, input, diff preview) on
///   <see cref="InitializeAsync"/> — unless a plugin has already registered a view with the
///   same id (override-before-builtin).</item>
///   <item>Auto-binds each view to its view model by matching
///   <see cref="ITuiView.Id"/> against <see cref="ITuiViewModel.Id"/>.</item>
///   <item>On every <see cref="RenderAsync"/>, fans the <see cref="AgentEvent"/> out to all
...
public abstract class BaseTuiRenderer : ITuiRenderer
{
    ...
    protected void RenderPlacement(TuiViewPlacement placement, AgentEvent @event)
    {
        ...
        foreach (var view in views)
        {
            try
            {
                view.RenderAsync(Context, default).GetAwaiter().GetResult();
            }
            catch (Exception ex)
            {
