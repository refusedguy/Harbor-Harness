...
public interface IAppConfigStore<T> where T : AppConfigBase
{
    ...
    public Task<Result<T>> LoadAsync(CancellationToken ct = default);
    ...
    public Task<Result> SaveAsync(T config, CancellationToken ct = default);
    ...
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Success, or failure with an error message.</returns>
    public Task<Result> UpdateAsync(Func<T, T> updater, CancellationToken ct = default);
}
