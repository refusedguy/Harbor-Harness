...
public interface ICommonConfigStore
{
    ...
    /// </returns>
    Task<Result<CommonConfig>> LoadAsync(CancellationToken ct = default);
    ...
    Task<Result> SaveAsync(CommonConfig config, CancellationToken ct = default);
    ...
    /// <param name="ct">Cancellation token.</param>
    /// <returns>Success, or failure with an error message.</returns>
    Task<Result> UpdateAsync(Func<CommonConfig, CommonConfig> updater, CancellationToken ct = default);
}
