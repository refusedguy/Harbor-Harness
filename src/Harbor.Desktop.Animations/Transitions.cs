    ...
    RgbColor ToColor)
{
    ...
    public RgbColor Interpolate(double t)
    {
        ...
        var r = (byte)(FromColor.R + (ToColor.R - FromColor.R) * clamped);
        var g = (byte)(FromColor.G + (ToColor.G - FromColor.G) * clamped);
        var b = (byte)(FromColor.B + (ToColor.B - FromColor.B) * clamped);
