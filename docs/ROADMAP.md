...
- [x] ROP-D final verification (25.08): `Harbor.slnx -c Release` builds with 0 errors; full cycle over all 20 `tests/Harbor.*.Tests` projects — 1184 tests: 1183 passed, 6 skipped, 1 failed = pre-existing flake `ChatView_Inflates` (61ee126). Enforcement matrix: 47/50 src dirs under arch rules; the other 3 documented out-of-scope (CodeGen build tool, Plugins.Host exe, Providers.Shared linked source) (commit b62de73).
