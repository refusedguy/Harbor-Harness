...
| Retry countdown (`RetryId`/`Attempt`/`ScheduledAt`) | request | `ToolDispatcher` retry loop (#43 `IToolRetryDecider` merged): publishes `ToolExecutionUpdateEvent` with `RetryAttempt`/`RetryMaxAttempts`/`RetryBackoffSeconds` before each backoff (`src/Harbor.Application/Agents/ToolDispatcher.cs`, #76); `RetryOptions` is config (`.../Resilience/RetryOptions.cs:3`); LLM-stream mirror unchanged (`AgentLoop.cs` `onRetry` logs → `PromptPipeline.ObserveEvent`) | None persistent | UI render-only, never triggers: `RetryCountdown.Line/Segments/BackoffSeconds` (`.../CellForge/Chat/Widgets/RetryCountdown.cs`); `StatusViewModel.Retry` slot (`.../Widgets/StatusViewModel.cs:24-28,103-106`); `ChatScreenBridge` mirrors retry updates, clears on end (`.../Streaming/ChatScreenBridge.cs`); `PromptPipeline.TryGetRetryProjection` → frame loop calls `SetProjectedRetry` (`CellForgeReplRunner.cs`, `PromptPipeline.cs`) | n/a (no state to recover) | Ephemeral per `ExecuteAsync` call |
...
2. **Retry countdown — DONE (#76).** Scheduler (`ToolDispatcher` retry loop over
   `IToolRetryDecider`, #43) owns attempt/schedule and publishes
   `ToolExecutionUpdateEvent` with `RetryAttempt`/`RetryMaxAttempts`/`RetryBackoffSeconds`
   before each backoff; UI renders remaining only, never triggers
   (`ChatScreenBridge` mirror + `PromptPipeline.TryGetRetryProjection` →
   `SetProjectedRetry`). Proofs: `ToolDispatcherRetryTests.RetryScheduled_*`;
   `ChatScreenBridgeTests.ToolRetryUpdate_*`; `ToolRetryProjectionTests`.
